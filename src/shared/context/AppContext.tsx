...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    try {
      ...
      setStatus((prev) => ({
        ...
        vitals:
          localVitals !== null ? JSON.parse(localVitals) : [true, true, true],
        ...prev,
        ...(localShutdown !== null
          ? { isShutdown: localShutdown === "true" }
          : {}),
  ...
  const updateShutdown = useCallback(async (shutdown: boolean) => {
    ...
    try {
      ...
      if (result.success) {
        setStatus((prev) =>
          prev ? { ...prev, isShutdown: result.isShutdown } : null,
        );
      }
    } catch {
      /* server will sync via polling */
    }
  }, []);

  const setBlackScreenOpacityState = useCallback(async (opacity: number) => {
    setStatus((prev) =>
      prev ? { ...prev, blackScreenOpacity: opacity } : null,
    );
    try {
      ...
      if (result.success) {
        setStatus((prev) =>
          prev
            ? { ...prev, blackScreenOpacity: result.blackScreenOpacity }
            : null,
        );
      }
    } catch {
  ...
  const setCameraState = useCallback(async (camera: number) => {
    ...
    if (result.success) {
      setStatus((prev) =>
        prev ? { ...prev, currentCamera: result.currentCamera } : null,
      );
    }
  }, []);
  ...
  return (
    <AppContext.Provider
      value={{
        ...
        error,
        updateShutdown,
        setBlackScreenOpacity: setBlackScreenOpacityState,
        setCamera: setCameraState,
        setPhase: setPhaseState,
        setVitals: setVitalsState,
